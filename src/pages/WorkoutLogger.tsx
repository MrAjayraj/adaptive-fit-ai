...
import { Check, ChevronLeft, Minus, Plus, Star, Timer, Trophy } from 'lucide-react';
import BottomNav from '@/components/layout/BottomNav';

export default function WorkoutLogger() {
  const { activeWorkoutId, currentPlan, updateWorkout, completeWorkout, getTodaysWorkout, startWorkout } = useFitness();
  ...
  // If no active workout, show a picker
  if (!workout) {
  ...
  if (showComplete) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-6 gap-6 animate-scale-in">
        <div className="w-20 h-20 rounded-full bg-gradient-primary flex items-center justify-center">
          <Trophy className="w-10 h-10 text-primary-foreground" />
        </div>
        <h2 className="text-2xl font-display font-bold text-foreground">Workout Complete!</h2>
        <p className="text-muted-foreground text-center">How did it feel?</p>
        ...
        <Button onClick={handleFinish} className="w-full max-w-xs bg-gradient-primary hover:opacity-90">
          Save & Finish
        </Button>
      </div>
    );
